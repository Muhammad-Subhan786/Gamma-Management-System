...
import { tasksAPI, employeeAPI } from '../services/api';
import { Calendar, Clock, StickyNote, User, CheckCircle, MessageCircle } from 'lucide-react';
...
const AdminTasksBoard = () => {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    const { title, description, due, dueTime, assignedTo, notes } = e.target.elements;
          ...
          <div className="modal-content max-w-md" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                <select name="assignedTo" defaultValue={modalTask?.assignedTo?._id || ''} className="input-field w-full" required>
            ...
            {modalTask && modalTask._id && (
              <div className="mt-6 border-t pt-4">
                ...
                <CommentsSection task={modalTask} employee={null} refreshTask={fetchTasks} />
              </div>
            )}
          </div>
        </div>
      ...
      <div className="space-y-2 max-h-40 overflow-y-auto mb-2">
        ...
        {comments.map(c => (
          <div key={c._id} className="flex items-start gap-2 bg-gray-50 rounded p-2">
            ...
            {/* Only show delete for admin or author. Admin can always delete. */}
...
export default AdminTasksBoard; 